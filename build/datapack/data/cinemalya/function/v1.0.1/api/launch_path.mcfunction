...
#> cinemalya:v1.0.1/api/launch_path
#
...
$execute if score #cinemalya.loaded load.status matches 1 if score #cinemalya.major load.status matches 1 if score #cinemalya.minor load.status matches 0 if score #cinemalya.patch load.status matches 1 run function cinemalya:v1.0.1/travel/from_waypoints {with:$(with)}
