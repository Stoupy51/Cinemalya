...
#> cinemalya:v1.0.1/load/check_dependencies
#
...
execute if score #dependency_error cinemalya.data matches 0 if score $bs.spline.major load.status matches 4 if score $bs.spline.minor load.status matches 1 unless score $bs.spline.patch load.status matches 1.. run scoreboard players set #dependency_error cinemalya.data 1
