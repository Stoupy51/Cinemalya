...
#> cinemalya:v1.0.1/intro/start
#
# @executed	positioned & rotated at the establishing shot the intro opens on
#
# @within	cinemalya:v1.0.1/api/intro {with:$(with)}
#
# @args		with (unknown)
#
...
## Optional arguments
execute unless data storage cinemalya:work intro.target_function run data modify storage cinemalya:work intro.target_function set value "cinemalya:v1.0.1/intro/none"
...
execute summon marker run function cinemalya:v1.0.1/intro/read_shot
kill @e[type=marker,tag=cinemalya.shot_ahead]

## Place every player, then start their travel from that shot
function cinemalya:v1.0.1/intro/spread with storage cinemalya:work intro
...
execute positioned ~ ~1.6 ~ run function cinemalya:v1.0.1/intro/display with storage cinemalya:work intro
