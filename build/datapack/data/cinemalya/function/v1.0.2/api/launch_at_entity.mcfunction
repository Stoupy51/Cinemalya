...
#> cinemalya:v1.0.2/api/launch_at_entity
#
...
$execute if score #cinemalya.loaded load.status matches 1 if score #cinemalya.major load.status matches 1 if score #cinemalya.minor load.status matches 0 if score #cinemalya.patch load.status matches 2 run function cinemalya:v1.0.2/travel/from_entity {with:$(with)}
