...
#> cinemalya:v1.0.3/api/stop
#
# @within	#cinemalya:v1/stop
#
# @args		with (unknown)
#
...
$execute if score #cinemalya.loaded load.status matches 1 if score #cinemalya.major load.status matches 1 if score #cinemalya.minor load.status matches 0 if score #cinemalya.patch load.status matches 3 run function cinemalya:v1.0.3/playback/stop {with:$(with)}
